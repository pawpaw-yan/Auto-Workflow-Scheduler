...
| **访问令牌** | `/api/user/self` 或前端缓存的 `localStorage.user` 里**名字带 `token` 的字段**（`access_token` / `accessToken` / …，各版本不一）。脚本会把所有候选**逐个真发请求验证**，谁能过就用谁 —— 不靠猜字段名。就是「个人设置 → 安全设置 → 系统访问令牌」那一串 |
...
- **访问令牌 / Cookie 两个框都可以直接编辑**：自动读不到时，把站点或 F12 里复制的值粘进去就行；
  粘过一次之后就不会再被自动读取覆盖
- 万一一个候选都验不过、或者根本没有 token 字段 → 说明这个版本**没把访问令牌下发给前端**。
  这不是 bug：去站点复制后粘进那个可编辑的框，或点「♻ 重新生成」拿一串新的。
  卡片里折叠的「**诊断：为什么读不到访问令牌**」会列出**它扫过哪些字段**，发我即可定位
