...
**⇄ 行格式转换** 按钮。点开粘行格式，选**输出形式**，再决定是「填入 SITES 输入框」还是「复制」：

| 输出形式 | 给谁用 |
|---|---|
| **SITES 值（不转义）** | 粘 Actions 页面的 `SITES` 输入框 —— 或者直接用「填入」按钮 |
| **HTTP body（带转义）** | cron-job.org / curl 的 Request body。**粘成上面那种就会报错**，差别只在转义 |
| **gh 命令（零转义）** | 整行复制到终端即可执行，不用手写转义 |
| 格式化预览 | 缩进版，**只看结构**（带真换行的，粘到 cron-job 会 400） |

- `ref` 输入框只在选「HTTP body / gh 命令」时出现（默认 `main`）
- 「填入」按钮只在选「**SITES 值**」时出现 —— 免得把整个 body 填进那个框

想从站点侧一连到底：在「账号小助手 → 提取账号」里点 **→ 填进 SITES JSON**，
行格式会**带着值**跳到这个卡片，选好形式复制即可。
...
登录站点后右上角会出现 **🛠 账号小助手**（也可以从篡改猴菜单里唤起）。点一下开二级菜单：

| 菜单项 | 做什么 |
|---|---|
| **提取账号** | 读出下面的用户 ID / 令牌 / Cookie，给出行格式与 SITES JSON |
| **SITES JSON** | 纯转换器：粘行格式 → 出 JSON，不需要打开 GitHub |

小助手**可以拖动，位置会记住**（存在该站点本地，默认右上角）。

| 取什么 | 从哪来 |
|---|---|
| 用户 ID | `GET /api/user/self`（`New-Api-User` 头是 new-api 的硬要求，先取 ID 再发请求） |
| **访问令牌** | 按下面顺序找，**前面能用就绝不碰后面**：<br>① `/api/user/self` / `localStorage.user` 里**名字带 `token` 的字段** —— 各版本字段名不一，所有候选都**逐个真发请求验证**，谁能过用谁<br>② 都不行，才调站点前端自己那个接口 **`GET /api/user/token`**（站点「访问令牌」弹窗就是靠它拿的。它是"有就返回原来的、没有才生成"，当读取用是安全的）<br>③ 还是不行才需要手工粘贴 |
| 会话 Cookie | 先试 `GM_cookie`（能读 httpOnly），不行退回 `document.cookie` |

卡片下半部分就是**提取到的原始值**：**用户 ID / 令牌 / Cookie**，每行一个「复制」。
要把它们变成行格式或 SITES JSON，走菜单里的「SITES JSON」（或 GitHub 派发页那个按钮）再粘一遍。

- ⚠️ **访问令牌 ≠ API 密钥**：`sk-` 开头、在「密钥管理」页里的那些是 **API 密钥**，给调用模型用的，
  **签到用不上** —— 脚本不碰它们，只取「个人设置 → 安全设置」里的系统访问令牌
- **访问令牌 / Cookie 两个框都可以直接编辑**：自动读不到时，把站点或 F12 里复制的值粘进去就行；
  粘过一次之后就不会再被自动读取覆盖
- 三条路都试过还拿不到（`/api/user/token` 也没给）→ 去站点「个人设置 → 安全设置」复制，
  粘进那个可编辑的框，粘完会自动验证。卡片里折叠的「**诊断：为什么读不到访问令牌**」
  会列出**它扫过哪些字段**，便于定位。真正的「重新生成」在站点那个按钮上点
- 「**🔍 测试访问令牌**」**真的发一次请求**：`GET /api/user/self` + `Authorization: Bearer <令牌>`
  （`credentials:'omit'`，不带会话 cookie，免得被浏览器会话"救活"造成假阳性）。
  这正是 api_checkin 签到流程调的第一个接口，所以它过了签到也能过。
  界面上会显示**这次请求的实际结果**（含返回的账号名），不再只给个 ✅
- 「**→ 填进 SITES JSON**」：访问令牌 / Cookie 至少要有一个，然后它直接**跳到 SITES JSON 卡片**
  并把行格式**填好**（访问令牌优先，没有才退到 Cookie）。省掉复制、切页、再粘贴三步
...
- **令牌可能是掩码**：`/api/token/` 列表有时只给 `sk-abc1********WXYZ` 这种打码值。
  脚本会再用 `GET /api/token/{id}` 取完整值，取不到就直说「去站点令牌页点复制」——
  不会拿一个掩码糊弄你
- **Cookie 可能读不到**：new-api 的会话 cookie 通常是 `httpOnly`，`document.cookie` 拿不到。
  脚本先试 `GM_cookie`（需篡改猴授权），仍拿不到就明说，并让你按 F12 → Network 复制
  Cookie 头（即上面「凭证怎么拿」的方式二）
- 令牌名（如 `api_checkin 2026-09-15`）只是**新建时取的标签**，不是令牌本身；
  令牌是站点签发、站点校验的，脚本只是帮你建和读
